...
test('configurator renders and updates the live build', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('$207.00')).toBeVisible()
  ...
  await expect(page.getByRole('status')).toContainText('Mock Shopify payload ready: SC-')
